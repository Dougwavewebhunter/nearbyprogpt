...
export const CategoriesGrid = () => {
  ...
  useEffect(() => {
    supabase.from("categories").select("id,name,slug,icon").order("name").then(({ data }) => setCats((data ?? []) as Cat[]));
    ...
    <section>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Pick a category to find trusted local pros.</p>
        </div>
